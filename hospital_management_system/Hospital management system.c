...
int visitCount = 0;
char doctor[100] = "Shashank Parchure";
char doctor_pass[100] = "iitj@2024";
char hospital_name[] = "IIT JODHPUR HOSPIAL";
char password[100];
int opt;
void baseline();
...
void visitRecord(char *path) {
    ...
    ptr = fopen(path, "r");
    if (ptr != NULL) {
        printf("Patient Id               Name                Doctor                Date\n");
        while (fgets(str, 100, ptr)) {
...
void updatePatient(char *path) {
    ...
    char name[100];
    char age[100];
    char dob[100]; // Not used currently
    char sex[100];
    char contact[100];
    char email[100];
    char address[100];
...
void extractText(FILE *ptr ,char *path, int i, char *name){
    ...
    char* t = strstr(text, ":") + 2;   //colon ke aage jaane ke liye 2 add 
    strcpy(name,t);
}
